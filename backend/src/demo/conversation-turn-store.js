...
export function createConversationTurnStore({ db }={}) {
  const completeTurn = async ({
    ...
    languagePolicy={}, callerTranscriptTurnId=null, agentTranscriptTurnId=null
  }) => {
    ...
    try {
      ...
      const inserted = await client.query(
        `INSERT INTO call_conversation_turns
          (turn_id,demo_call_id,turn_sequence,caller_text,agent_text,language,language_policy,caller_transcript_turn_id,agent_transcript_turn_id)
         VALUES ($1,$2,$3,$4,$5,$6,$7::jsonb,$8,$9)
         ON CONFLICT (turn_id) DO NOTHING
         RETURNING *`,
        [turnId,demoCallId,turnSequence,String(callerText),String(agentText),language,JSON.stringify(languagePolicy || {}),callerTranscriptTurnId,agentTranscriptTurnId]
      )
  ...
  const loadLatestCompletedTurns = async ({ demoCallId, limit=16 }={}) => {
    ...
    const result = await db.query(
      `SELECT turn_sequence,caller_text,agent_text,language,language_policy,completed_at
