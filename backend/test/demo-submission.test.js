...
test("records a valid submission that is blocked by the rate limit", async () => {
  const queries = []
  const db = { async query(sql, values = []) {
    queries.push({ sql, values })
    if (sql.includes("SELECT COUNT(*)")) return { rows:[{ count:3 }], rowCount:1 }
    return { rows:[], rowCount:1 }
  }}
  const service = createDemoService({
    db, limiter:{ consume:async () => ({ allowed:true }) }, plivo:{}, twilio:{}, followupQueue:{ add:async () => {} },
  ...
  assert.ok(queries.some(({ sql, values }) => sql.includes("UPDATE demo_submissions") && values[1] === "rate_limited" && values[2] === "phone_limit"))
})
