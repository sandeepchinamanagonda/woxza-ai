...
test("Anthropic V3 adapter streams caller-facing text with compact call memory", async () => {
  ...
  const chunks = []
  let completion
  for await (const chunk of client.replyStream({ language:"te", history:[], callerText:"హలో", memory:{ turns:[] } })) { if (chunk.text) chunks.push(chunk.text); if (chunk.completion) completion = chunk.completion }
  ...
  assert.deepEqual(completion, { stopReason:"end_turn", stopSequence:null, usage:{ output_tokens:12 } })
})
