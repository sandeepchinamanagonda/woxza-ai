...
test("V3 conversation prompt keeps the behavioural Woxza contract", () => {
  ...
  assert.match(prompt, /Never choose, infer, or introduce a business type/i)
  assert.match(prompt, /If the caller has not yet named their business, ask one neutral question/i)
  ...
  assert.match(prompt, /four distinct practical capabilities/i)
  assert.match(prompt, /four more different capabilities/i)
  assert.match(prompt, /exactly six complete spoken sentences/i)
  assert.match(prompt, /four_more_examples/i)
  ...
  assert.match(prompt, /Onboarding and Waitlist/i)
  assert.match(prompt, /Woxza AI website, woxza\.ai/i)
  assert.match(prompt, /must both be spoken in every onboarding answer/i)
  assert.match(prompt, /product updates, and pricing information/i)
  assert.match(prompt, /Ending a Demo/i)
  assert.match(prompt, /clear goodbye, thanks followed by an intention to leave/i)
})
...
test("V3 discovery avoids a fixed interview while retaining facts silently", () => {
  ...
  assert.match(prompt, /Questions are optional, not a requirement for every turn/i)
  assert.match(prompt, /Do not move through business type, location, channels, team, pain, or goals in a fixed order/i)
  assert.match(prompt, /they already told you, that you are repeating yourself, or asks why you are asking, treat it as pushback/i)
  assert.match(prompt, /When the Caller Pushes Back/i)
  assert.match(prompt, /A specific factual or technical question about a named integration, POS system, or price is not a full-picture trigger/i)
