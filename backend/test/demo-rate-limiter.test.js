...
test("limits a phone for 30 minutes without consuming unrelated limits", async () => {
  ...
  assert.deepEqual(await limiter.consume({ phone:"+13125550100", ip:"1.1.1.1" }), { allowed:true })
  assert.deepEqual(await limiter.consume({ phone:"+13125550100", ip:"1.1.1.2" }), { allowed:false, reason:"phone_limit" })
  timestamp += 30 * 60 * 1000 + 1
  assert.deepEqual(await limiter.consume({ phone:"+13125550100", ip:"1.1.1.2" }), { allowed:true })
})

test("limits an IP to three calls and enforces the global cap", async () => {
  ...
  for (let index=0; index<3; index += 1) assert.equal((await ipLimiter.consume({ phone:`+1312555010${index}`, ip:"2.2.2.2" })).allowed,true)
